...
class Bottleneck_GMR(nn.Module):
    """Standard bottleneck."""
    ...
    def __init__(
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = GMR_Conv2d(c_, c2, k[1], 1, 1)
        self.add = shortcut and c1 == c2
...
class C2fGMR(C2f):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1,
        ...
        self.m = nn.Sequential(*(Bottleneck_GMR(c_, c_, shortcut, g, e=1.0)for _ in range(n)))


if __name__ == "__main__":
    x = torch.randn(1, 256, 40, 40)

    model = C2fGMR(c1=256, c2=256, n=2, shortcut=True, g=1, e=0.5)
    y = model(x)
