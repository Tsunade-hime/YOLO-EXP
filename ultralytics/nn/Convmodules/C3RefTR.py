...
class C3RefTR(C3):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5):
        ...
        c_ = int(c2 * e)
        self.cv1 = RefConv(c1, c_, 1, 1)  # 第一层卷积，1x1卷积
        self.cv2 = RefConv(c1, c_, 1, 1)  # 第二层卷积，1x1卷积
        self.cv3 = RefConv(2 * c_, c2, 1, 1)  # 第三层卷积，1x1卷积，连接后的结果会通过该卷积层输出
        self.m = TransformerBlock(c_, c_, 4, n)

if __name__ == "__main__":
    x = torch.randn(1, 256, 40, 40)

    model = C3RefTR(c1=256, c2=256, n=2, shortcut=True, g=1, e=0.5)
    y = model(x)
