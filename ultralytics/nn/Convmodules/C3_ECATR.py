...
class CBS_ECA(nn.Module):
    def __init__(self, in_ch, out_ch, eca_k_size=None):
        super().__init__()
        self.conv = Conv(in_ch, out_ch, 1, 1)   # 1x1 CBS
        self.eca = ECA(out_ch, k_size=eca_k_size)
...
class C3ECATR(C3):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5):
        ...
        c_ = int(c2 * e)
        self.cv2 = CBS_ECA(c1, c_)  # 第二层卷积，1x1卷积
        self.m = TransformerBlock(c_, c_, 4, n)

if __name__ == "__main__":
    x = torch.randn(1, 256, 40, 40)

    model = C3ECATR(c1=256, c2=256, n=2, shortcut=True, g=1, e=0.5)
    y = model(x)
